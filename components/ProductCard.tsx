...
export function ProductCard({ produto }: ProductCardProps) {
  ...
  return (
    ...
    >
      {/* Foto */}
      <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-xl overflow-hidden bg-secondary/60 flex items-center justify-center flex-shrink-0">
        {imgSrc ? (
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <h3 className="font-semibold text-foreground text-base leading-snug">{produto.nome}</h3>
        {produto.descricao && (
        ...
        <div className="mt-3 pt-1">
          {ehKg && modoSel === 'kg' ? (
            noCarrinho === 0 ? (
              <div className="flex flex-wrap gap-2">
                {PESOS.map((w) => (
                  <button
                    key={w}
                    onClick={() => definirQuantidade(produto, 'kg', w)}
                    disabled={semEstoque}
            ...
            <button
              onClick={() => adicionarItem(produto, 'un')}
              disabled={semEstoque}
